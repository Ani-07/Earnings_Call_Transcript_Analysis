...
file = pd.read_csv("file_test.csv")
...
vocab = EC_merged.columns

#file_count = 1

#Open the relevant matrix file

EC_Count = EC_Count = pd.DataFrame(columns = EC_merged.columns)
...
for j in range(file_names.shape[0]):
    ...
    vec_file = count_vec(vocab, tokens)
...
matrix_name = "Test_Matrix.csv"
EC_Count.to_csv(matrix_name, index = False)

#new = file.drop(errors)
#new.to_csv("Test_Updated.csv", index = False)
